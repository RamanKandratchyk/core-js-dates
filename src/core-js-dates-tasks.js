...
function getNextFriday(date) {
  ...
  const nextFridayPeriod = 5 - currDay > 0 ? 5 - currDay : 12 - currDay;
  date.setDate(date.getDate() + nextFridayPeriod);
  return date;
}
...
function getCountWeekendsInMonth(month, year) {
  let weekdaysCount = 0;
  let currDay = 1;
  const currDate = new Date(year, month - 1, currDay);
  do {
...
function getWeekNumberByDate(date) {
  ...
  const inputTime = date.getTime() - timeOffset;

  const firstDateInYear = new Date(date.getUTCFullYear(), 0, 1);
  const firstDateInYearOffset = new Date(
